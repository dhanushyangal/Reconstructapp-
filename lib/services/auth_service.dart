...
class AuthService extends ChangeNotifier {
  ...
  static Future<void> enableGuestMode() async {
    isGuest = true;
    // Save guest mode to SharedPreferences for persistence
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('is_guest_mode', true);
      debugPrint('Guest mode enabled and saved to SharedPreferences');
    } catch (e) {
      debugPrint('Error saving guest mode to SharedPreferences: $e');
    }
  }
  static Future<void> signOutGuest() async {
    ...
    try {
      ...
      debugPrint('Guest mode disabled and cleared from SharedPreferences');
    } catch (e) {
      debugPrint('Error clearing guest mode from SharedPreferences: $e');
    }
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      debugPrint('AuthService: Checking if Supabase user is authenticated: ${_supabaseService.isAuthenticated}');
      if (_supabaseService.isAuthenticated) {
        final profileResult = await _supabaseService.getUserProfile();
        debugPrint('AuthService: Profile result: ${profileResult['success']}');
        if (profileResult['success'] == true) {
          _userData = profileResult['user'];
          debugPrint('AuthService: User already authenticated and profile loaded');
        } else {
          debugPrint('AuthService: Failed to load user profile: ${profileResult['message']}');
        }
      } else {
        debugPrint('AuthService: No authenticated Supabase user found');
      }
    } catch (e) {
  ...
  bool hasAuthenticatedUser() {
    final supabaseAuth = _supabaseService.isAuthenticated;
    final hasUserData = _userData != null;
    debugPrint('AuthService: hasAuthenticatedUser() - Supabase auth: $supabaseAuth, User data: $hasUserData');
    return supabaseAuth && hasUserData;
  }
  ...
  Future<Map<String, dynamic>> signInWithEmailPassword({
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (isGuest) {
          await signOutGuest();
          debugPrint('🔐 AuthService: Cleared guest mode after successful login');
        }
  ...
  Future<dynamic> signInWithGoogle() async {
    try {
      ...
      if (result['success'] == true) {
        ...
        if (isGuest) {
          await signOutGuest();
          debugPrint('AuthService: Cleared guest mode after successful Google login');
        }
  ...
  Future<Map<String, dynamic>> registerUser({
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        // Clear guest mode when user successfully registers
        if (isGuest) {
          await signOutGuest();
          debugPrint('AuthService: Cleared guest mode after successful registration');
        }
  ...
  // Check if user is authenticated
  bool get isAuthenticated => isGuest || hasAuthenticatedUser();
