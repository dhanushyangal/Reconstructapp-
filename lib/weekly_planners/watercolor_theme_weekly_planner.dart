...
class WatercolorThemeWeeklyPlanner extends StatefulWidget {
  final int dayIndex;
  final String? eventId;
  final bool showEvents;

  const WatercolorThemeWeeklyPlanner({
    super.key,
  ...
  State<WatercolorThemeWeeklyPlanner> createState() =>
      _WatercolorThemeWeeklyPlannerState();
}

class _WatercolorThemeWeeklyPlannerState
    extends State<WatercolorThemeWeeklyPlanner> with ActivityTrackerMixin {
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var day in days) {
      try {
        final savedTodos = prefs.getString('WatercolorTheme_todos_$day');
        if (savedTodos != null) {
        ...
        try {
          final widgetTodos =
              await HomeWidget.getWidgetData('WatercolorTheme_todos_$day');
          if (widgetTodos != null) {
        ...
        if (_todoLists[day]?.isNotEmpty == true) {
          await HomeWidget.saveWidgetData(
              'watercolor_todo_text_$day', _formatDisplayText(day));
        }
      } catch (e) {
  ...
  Future<void> _syncWithDatabase() async {
    ...
    try {
      ...
      if (userInfo['userName']?.isNotEmpty == true &&
        ...
        // Explicitly request watercolor theme tasks only
        final allTasksFromDb = await WeeklyPlannerService.instance
            .loadUserTasks(userInfo, theme: 'watercolor');

        if (allTasksFromDb.isNotEmpty) {
          final prefs = await SharedPreferences.getInstance();

          // Process only tasks that belong to the watercolor theme
          for (var dbTask in allTasksFromDb) {
            if (dbTask['theme'] != 'watercolor') {
            ...
            if (cardId.startsWith('watercolor_')) {
              // New format with theme prefix
              day = cardId.substring('watercolor_'.length);
            ...
            if (_todoLists.containsKey(day)) {
              try {
                ...
                await prefs.setString('WatercolorTheme_todos_$day', tasksJson);
                ...
                final widgetEncoded = json.encode(_todoLists[day]!
                    ...
                    .toList());
                await HomeWidget.saveWidgetData(
                    'WatercolorTheme_todos_$day', widgetEncoded);
                await HomeWidget.saveWidgetData(
                    'watercolor_todo_text_$day', _formatDisplayText(day));
  ...
  Future<void> _saveTodoList(String day) async {
    ...
    try {
      ...
      await prefs.setString('WatercolorTheme_todos_$day', encoded);
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await WeeklyPlannerService.instance
              .saveTodoItem(userInfo, cardId, encoded, theme: 'watercolor');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/watercolor_weekly_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Watercolor Theme Weekly Planner');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Watercolor Theme Weekly Planner'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            children: [
              Positioned.fill(
                child: Image.asset(
                  'assets/watercolor/watercolor_${dayIndex + 1}.png',
                  fit: BoxFit.cover,
                ),
              ),
              Column(
