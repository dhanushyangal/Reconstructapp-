...
class JapaneseThemeWeeklyPlanner extends StatefulWidget {
  final int dayIndex;
  final String? eventId;
  final bool showEvents;

  const JapaneseThemeWeeklyPlanner({
    super.key,
  ...
  static const routeName = '/japanese-theme-weekly-planner';

  @override
  State<JapaneseThemeWeeklyPlanner> createState() =>
      _JapaneseThemeWeeklyPlannerState();
}

class _JapaneseThemeWeeklyPlannerState extends State<JapaneseThemeWeeklyPlanner>
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var day in days) {
      try {
        final savedTodos = prefs.getString('JapaneseTheme_todos_$day');
        if (savedTodos != null) {
        ...
        try {
          final widgetTodos =
              await HomeWidget.getWidgetData('JapaneseTheme_todos_$day');
          if (widgetTodos != null) {
        ...
        if (_todoLists[day]?.isNotEmpty == true) {
          await HomeWidget.saveWidgetData(
              'japanese_todo_text_$day', _formatDisplayText(day));
        }
      } catch (e) {
  ...
  Future<void> _syncWithDatabase() async {
    ...
    try {
      ...
      if (userInfo['userName']?.isNotEmpty == true &&
        ...
        // Explicitly request japanese theme tasks only
        final allTasksFromDb = await WeeklyPlannerService.instance
            .loadUserTasks(userInfo, theme: 'japanese');

        if (allTasksFromDb.isNotEmpty) {
          final prefs = await SharedPreferences.getInstance();

          // Process only tasks that belong to the japanese theme
          for (var dbTask in allTasksFromDb) {
            if (dbTask['theme'] != 'japanese') {
            ...
            if (cardId.startsWith('japanese_')) {
              // New format with theme prefix
              day = cardId.substring('japanese_'.length);
            ...
            if (_todoLists.containsKey(day)) {
              try {
                ...
                await prefs.setString('JapaneseTheme_todos_$day', tasksJson);
                ...
                final widgetEncoded = json.encode(_todoLists[day]!
                    ...
                    .toList());
                await HomeWidget.saveWidgetData(
                    'JapaneseTheme_todos_$day', widgetEncoded);
                await HomeWidget.saveWidgetData(
                    'japanese_todo_text_$day', _formatDisplayText(day));
  ...
  Future<void> _saveTodoList(String day) async {
    try {
      ...
      await prefs.setString('JapaneseTheme_todos_$day', encoded);
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await WeeklyPlannerService.instance
              .saveTodoItem(userInfo, cardId, encoded, theme: 'japanese');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/japanese_weekly_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Japanese Theme Weekly Planner');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Japanese Theme Weekly Planner'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            children: [
              Positioned.fill(
                child: Image.asset(
                  'assets/japaness_weekly/japanese${dayIndex + 1}.png',
                  fit: BoxFit.cover,
                ),
              ),
              Column(
  ...
  Future<void> _trackActivity() async {
    try {
      final activity = RecentActivityItem(
        ...
        imagePath: 'assets/japanese_theme_weekly_planner.png',
        timestamp: DateTime.now(),
        routeName: JapaneseThemeWeeklyPlanner.routeName,
      );
