...
class FloralThemeWeeklyPlanner extends StatefulWidget {
  final int dayIndex;
  final String? eventId;
  final bool showEvents;

  const FloralThemeWeeklyPlanner({
    super.key,
...
class _FloralThemeWeeklyPlannerState extends State<FloralThemeWeeklyPlanner>
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var day in days) {
      try {
        final savedTodos = prefs.getString('FloralTheme_todos_$day');
        if (savedTodos != null) {
        ...
        try {
          final widgetTodos =
              await HomeWidget.getWidgetData('FloralTheme_todos_$day');
          if (widgetTodos != null) {
        ...
        if (_todoLists[day]?.isNotEmpty == true) {
          await HomeWidget.saveWidgetData(
              'floral_todo_text_$day', _formatDisplayText(day));
        }
      } catch (e) {
  ...
  Future<void> _syncWithDatabase() async {
    ...
    try {
      ...
      if (userInfo['userName']?.isNotEmpty == true &&
        ...
        // Explicitly request floral theme tasks only
        ...
        if (allTasksFromDb.isNotEmpty) {
          ...
          for (var dbTask in allTasksFromDb) {
            ...
            if (cardId.startsWith('floral_')) {
              // New format with theme prefix
              day = cardId.substring('floral_'.length);
            ...
            if (_todoLists.containsKey(day)) {
              try {
                ...
                await prefs.setString('FloralTheme_todos_$day', tasksJson);
                ...
                final widgetEncoded = json.encode(_todoLists[day]!
                    ...
                    .toList());
                await HomeWidget.saveWidgetData(
                    'FloralTheme_todos_$day', widgetEncoded);
                await HomeWidget.saveWidgetData(
                    'floral_todo_text_$day', _formatDisplayText(day));
  ...
  Future<void> _saveTodoList(String day) async {
    try {
      ...
      await prefs.setString('FloralTheme_todos_$day', encoded);
      ...
      // Save to HomeWidget
      await HomeWidget.saveWidgetData('FloralTheme_todos_$day', widgetEncoded);
      await HomeWidget.saveWidgetData(
          'floral_todo_text_$day', _formatDisplayText(day));
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await WeeklyPlannerService.instance
              .saveTodoItem(userInfo, cardId, encoded, theme: 'floral');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/floral_weekly_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Floral Theme Weekly Planner');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Floral Theme Weekly Planner'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            children: [
              Positioned.fill(
                child: Image.asset(
                  'assets/floral_weekly/floral_${dayIndex + 1}.png',
                  fit: BoxFit.cover,
                ),
              ),
              Column(
