...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  Future<void> _initializeAuth() async {
    ...
    // Initialize AuthService first to load user data if authenticated
    await _authService.initialize();
    debugPrint('AuthService initialized. isAuthenticated: ${_authService.isAuthenticated}');
    
    // First check if user is authenticated - this takes priority over guest mode
    if (_authService.isAuthenticated) {
      // User is authenticated, clear any guest mode state
      if (wasGuestMode) {
        await prefs.remove('is_guest_mode');
        AuthService.isGuest = false;
        debugPrint('Cleared guest mode because user is authenticated');
      }
    ...
    // If not authenticated, check if we should restore guest mode
    if (wasGuestMode) {
      // Restore guest mode
      AuthService.isGuest = true;
      debugPrint('Restored guest mode from SharedPreferences');
      setState(() => _isInitializing = false);
      return;
    }
    
    // No authentication and no guest mode
    setState(() => _isInitializing = false);
  }
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: const Text('This feature requires you to sign in to your account. '
                'Please sign in to access all features and save your progress.'),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(context).pop(),
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _ensureProfileDataLoaded() async {
    // For guest users, no need to load additional data
    if (widget.isGuest || AuthService.isGuest) {
      return;
    }
  ...
  Future<void> _loadProfileData() async {
    ...
    if (widget.isGuest || AuthService.isGuest) {
      debugPrint('ProfilePage: Loading guest profile data');
      setState(() {
        _displayName = 'Guest';
        _userEmail = 'guest@reconstruct.app';
        _userId = 'guest_user';
  ...
  Future<void> loadPremiumStatus() async {
    try {
      // For guest users, set premium status to false
      if (widget.isGuest || AuthService.isGuest) {
        debugPrint('Guest user detected - setting premium status to false');
        if (mounted) {
          setState(() {
            _isPremium = false;
          });
          debugPrint('Guest user premium status set to false');
        }
        return;
      }
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: (widget.isGuest || AuthService.isGuest) ? Colors.blue : Colors.red[700],
                                  foregroundColor: Colors.white,
                                  padding:
                                ...
                                onPressed: (widget.isGuest || AuthService.isGuest) ? _signIn : _signOut,
                                icon: Icon((widget.isGuest || AuthService.isGuest) ? Icons.login : Icons.logout),
                                label: Text((widget.isGuest || AuthService.isGuest) ? 'Sign In' : 'Sign Out'),
                              ),
                            ),
                            if (!(widget.isGuest || AuthService.isGuest)) ...[
                              const SizedBox(height: 16),
  ...
  Future<void> _pickImage() async {
    // Guest users cannot pick profile images
    if (widget.isGuest || AuthService.isGuest) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please sign in to change your profile picture'),
  ...
  Future<void> _signOut() async {
    ...
    if (confirm) {
      try {
        ...
        if (AuthService.isGuest) {
          await AuthService.signOutGuest();
        }
  ...
  Future<bool> _isUserOnTrial() async {
    try {
      // For guest users, always return false (no trial for guests)
      if (widget.isGuest || AuthService.isGuest) {
        debugPrint(
            'ProfilePage: Guest user - trial check returns false (no trial for guests)');
        return false;
      }
  ...
  Future<String> _getTrialEndDate() async {
    try {
      // For guest users, return empty string (no trial end date needed)
      if (widget.isGuest || AuthService.isGuest) {
        debugPrint('ProfilePage: Guest user - no trial end date needed');
        return '';
      }
  ...
  Future<void> _showPaymentPage() async {
    try {
      // For guest users, prompt them to sign in first
      if (widget.isGuest || AuthService.isGuest) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Please sign in to access premium features'),
              backgroundColor: Colors.orange,
              duration: Duration(seconds: 3),
            ),
          );
