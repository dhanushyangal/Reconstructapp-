...
class WatercolorThemeAnnualPlanner extends StatefulWidget {
  final int monthIndex;
  final String? eventId;
  final bool showEvents;

  const WatercolorThemeAnnualPlanner({
    super.key,
  ...
  State<WatercolorThemeAnnualPlanner> createState() =>
      _WatercolorThemeAnnualPlannerState();
}

class _WatercolorThemeAnnualPlannerState
    extends State<WatercolorThemeAnnualPlanner> with ActivityTrackerMixin {
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var month in months) {
      try {
        final savedTodos = prefs.getString('WatercolorTheme_todos_$month');
        if (savedTodos != null) {
            ...
            if (_todoLists.containsKey(month)) {
              try {
                ...
                await prefs.setString(
                    'WatercolorTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
                    'WatercolorTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
                    'watercolor_todo_text_$month', _formatDisplayText(month));
  ...
  Future<void> _saveTodoList(String month) async {
    try {
      ...
      await prefs.setString('WatercolorTheme_todos_$month', encoded);

      // Save to HomeWidget
      await HomeWidget.saveWidgetData('WatercolorTheme_todos_$month', encoded);
      await HomeWidget.saveWidgetData(
          'watercolor_todo_text_$month', _formatDisplayText(month));
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await AnnualCalendarService.instance
              .saveTodoItem(userInfo, month, encoded, theme: 'watercolor');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/watercolor_annual_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Watercolor Theme Annual Planner for 2025');
    } catch (e) {
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          image: DecorationImage(
            image: AssetImage(
                'assets/watercolor/watercolor_${monthIndex + 1}.png'),
            fit: BoxFit.cover,
          ),
          boxShadow: [
            BoxShadow(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Watercolor Theme Annual Planner 2025'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
