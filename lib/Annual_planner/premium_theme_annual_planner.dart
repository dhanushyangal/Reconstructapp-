...
class PremiumThemeAnnualPlanner extends StatefulWidget {
  final int monthIndex;
  final String? eventId;
  final bool showEvents;

  const PremiumThemeAnnualPlanner({
    super.key,
  ...
  State<PremiumThemeAnnualPlanner> createState() =>
      _PremiumThemeAnnualPlannerState();
}

class _PremiumThemeAnnualPlannerState extends State<PremiumThemeAnnualPlanner>
  ...
  void initState() {
    ...
    trackUserInteraction('click', details: 'View Annual Planner');
  }
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var month in months) {
      try {
        final savedTodos = prefs.getString('PremiumTheme_todos_$month');
        if (savedTodos != null) {
    ...
    try {
      ...
      if (userInfo['userName']?.isNotEmpty == true &&
        ...
        final allTasksFromDb = await AnnualCalendarService.instance
            .loadUserTasks(userInfo, theme: 'premium');
          ...
          for (var dbTask in allTasksFromDb) {
            ...
            if (_todoLists.containsKey(month)) {
              try {
                ...
                await prefs.setString('PremiumTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
                    'PremiumTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
                    'premium_todo_text_$month', _formatDisplayText(month));
  ...
  Future<void> _saveTodoList(String month) async {
    try {
      ...
      await prefs.setString('PremiumTheme_todos_$month', encoded);

      // Save to HomeWidget
      await HomeWidget.saveWidgetData('PremiumTheme_todos_$month', encoded);
      await HomeWidget.saveWidgetData(
          'premium_todo_text_$month', _formatDisplayText(month));
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await AnnualCalendarService.instance
              .saveTodoItem(userInfo, month, encoded, theme: 'premium');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/premium_annual_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Premium Theme Annual Planner for 2025');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Premium Theme Annual Planner 2025'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
  ...
  void _addTask(String month, String task) {
    // ... existing logic ...
    trackButtonTap('Add Task', additionalDetails: 'month:$month task:$task');
  }
}
