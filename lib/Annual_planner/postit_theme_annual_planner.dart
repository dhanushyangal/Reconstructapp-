...
class PostItThemeAnnualPlanner extends StatefulWidget {
  final int monthIndex;
  final String? eventId;
  final bool showEvents;

  const PostItThemeAnnualPlanner({
    super.key,
  ...
  static const routeName = '/postit-theme-annual-planner';

  @override
  State<PostItThemeAnnualPlanner> createState() =>
...
class _PostItThemeAnnualPlannerState extends State<PostItThemeAnnualPlanner>
  ...
  final List<Color> cardColors = [
    Color(0xFFFF7F6A), // January - Coral
    Color(0xFFFFB347), // February - Orange
    Color(0xFFFFB5B5), // March - Pink
    Color(0xFF4169E1), // April - Royal Blue
    Color(0xFF87CEEB), // May - Sky Blue
    Color(0xFFFFF0F5), // June - Light Pink
    Color(0xFFFFFF00), // July - Yellow
    Color(0xFFFF69B4), // August - Hot Pink
    Color(0xFF00CED1), // September - Turquoise
    Color(0xFFFF69B4), // October - Pink Purple
    Color(0xFF4169E1), // November - Royal Blue
    Color(0xFFFF6B6B), // December - Red Orange
  ];
  ...
  Future<void> _trackActivity() async {
    try {
      final activity = RecentActivityItem(
        name: 'Post-it Theme Annual Planner',
        imagePath: 'assets/images/postit.png',
        timestamp: DateTime.now(),
        routeName: PostItThemeAnnualPlanner.routeName,
      );
  ...
  Future<void> _loadAllFromLocalStorage() async {
    ...
    for (var month in months) {
      try {
        final savedTodos = prefs.getString('PostItTheme_todos_$month');
        if (savedTodos != null) {
    ...
    try {
      ...
      if (userInfo['userName']?.isNotEmpty == true &&
        ...
        final allTasksFromDb = await AnnualCalendarService.instance
            .loadUserTasks(userInfo, theme: 'postit');
          ...
          for (var dbTask in allTasksFromDb) {
            ...
            if (_todoLists.containsKey(month)) {
              try {
                ...
                await prefs.setString('PostItTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
                    'PostItTheme_todos_$month', tasksJson);
                await HomeWidget.saveWidgetData(
  ...
  Future<void> _saveTodoList(String month) async {
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text('Please sign in to save your annual planner.'),
            actions: [
              TextButton(
    ...
    try {
      ...
      await prefs.setString('PostItTheme_todos_$month', encoded);

      // Save to HomeWidget
      await HomeWidget.saveWidgetData('PostItTheme_todos_$month', encoded);
      await HomeWidget.saveWidgetData(
          'postit_todo_text_$month', _formatDisplayText(month));
      ...
      try {
        ...
        if (userInfo['userName']?.isNotEmpty == true &&
          ...
          final success = await AnnualCalendarService.instance
              .saveTodoItem(userInfo, month, encoded, theme: 'postit');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/postit_annual_planner.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My PostIt Theme Annual Planner for 2025');
    } catch (e) {
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: cardColors[monthIndex], // Use fixed color based on month
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('PostIt Theme Annual Planner 2025'),
        actions: [
          // Add a sync button
          _isSyncing
              ? const Padding(
