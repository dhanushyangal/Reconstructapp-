...
class SportThemeVisionBoard extends StatefulWidget {
  const SportThemeVisionBoard({super.key});

  @override
  State<SportThemeVisionBoard> createState() => _SportThemeVisionBoardState();
}

class _SportThemeVisionBoardState extends State<SportThemeVisionBoard> {
  ...
  Future<void> _loadSavedData(String category) async {
    ...
    // Load todos
    final savedTodos = prefs.getString('sport_todos_$category');
    if (savedTodos != null) {
  ...
  Future<void> _saveTodoList(String category) async {
    ...
    await prefs.setString('sport_todos_$category', encoded);
    await HomeWidget.saveWidgetData('sport_todos_$category', encoded);
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/sport_vision_board.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Sport Vision Board for 2025');
    } catch (e) {
      ...
      child: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
            image: AssetImage(
                'assets/sports/geometry-${visionCategories.indexOf(title) + 1}.png'),
            fit: BoxFit.cover,
          ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Sport Theme Vision Board'),
      ),
      body: Column(
        children: [
          Expanded(
