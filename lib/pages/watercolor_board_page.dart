...
class WatercolorThemeVisionBoard extends StatefulWidget {
  const WatercolorThemeVisionBoard({super.key});

  @override
  State<WatercolorThemeVisionBoard> createState() =>
      _WatercolorThemeVisionBoardState();
}

class _WatercolorThemeVisionBoardState
    extends State<WatercolorThemeVisionBoard> {
  ...
  Future<void> _loadSavedData(String category) async {
    ...
    // Load todos
    final savedTodos = prefs.getString('watercolor_todos_$category');
    if (savedTodos != null) {
  ...
  Future<void> _saveTodoList(String category) async {
    ...
    await prefs.setString('watercolor_todos_$category', encoded);
    await HomeWidget.saveWidgetData('watercolor_todos_$category', encoded);
  }
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/watercolor_vision_board.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Watercolor Vision Board for 2025');
    } catch (e) {
  ...
  Widget _buildVisionCard(String title, int index) {
    String backgroundImage = 'assets/watercolour/watercolor-${index + 1}.png';
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  onChanged: (value) async {
                    final prefs = await SharedPreferences.getInstance();
                    await prefs.setString('watercolor_vision_$title', value);
                  },
                ),
              ),
            ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Watercolor Theme Vision Board'),
      ),
      body: Column(
        children: [
          Expanded(
