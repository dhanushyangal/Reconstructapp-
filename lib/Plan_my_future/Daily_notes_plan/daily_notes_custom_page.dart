...
class DailyNotesCustomPage extends StatefulWidget {
  ...
  @override
  State<DailyNotesCustomPage> createState() => _DailyNotesCustomPageState();
}

class _DailyNotesCustomPageState extends State<DailyNotesCustomPage> {
  ...
  Color get _themeBackgroundColor {
    switch (widget.template) {
      case 'Post-it Daily Notes':
        return Colors.lightGreen.shade100; // Light green for Post-it
      case 'Premium Daily Notes':
        return Colors.black; // Black for Premium
      case 'Floral Daily Notes':
        return Colors.white; // White for Floral (will use image)
      default:
        return Colors.white;
    }
  }

  // Check if theme should use image background
  bool get _shouldUseImageBackground {
    return widget.template == 'Floral Daily Notes';
  }

  // Get theme-specific background image
  String get _themeBackgroundImage {
    return 'assets/floral_weekly/floral_1.png';
  }
  ...
  Color get _themeTextColor {
    switch (widget.template) {
      case 'Premium Daily Notes':
        return Colors.white; // White text for black background
      default:
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _shouldUseImageBackground ? Colors.white : _themeBackgroundColor,
          border: Border.all(color: Colors.grey.shade300),
          borderRadius: BorderRadius.circular(4),
          image: _shouldUseImageBackground ? DecorationImage(
            image: AssetImage(_themeBackgroundImage),
            fit: BoxFit.cover,
          ) : null,
        ),
        child: ClipRRect(
          ...
          child: Column(
            ...
            children: [
              ...
              // Content container with conditional overlay
              Container(
                padding: const EdgeInsets.all(12.0),
                decoration: _shouldUseImageBackground ? BoxDecoration(
...
class _NoteEditorPageState extends State<NoteEditorPage> {
  ...
  // Theme-specific background colors for editor
  Color get _themeBackgroundColor {
    switch (widget.template) {
      case 'Post-it Daily Notes':
      ...
      case 'Premium Daily Notes':
        return Colors.black;
      case 'Floral Daily Notes':
        return Colors.white; // Will use image
      default:
        return Colors.white;
    }
  }
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            // Semi-transparent overlay (only for image backgrounds)
            if (_shouldUseImageBackground)
              Container(
