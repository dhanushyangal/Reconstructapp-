...
// Replace colorOptions with categoryColors
  ...
  Future<void> _saveDates() async {
    try {
      ...
      await Future.wait([
        prefs.setString('spaniel.calendar_theme_2025', jsonData),
        prefs.setString('spaniel.calendar_data', jsonData),
        HomeWidget.saveWidgetData('spaniel.calendar_data', jsonData),
        HomeWidget.saveWidgetData('spaniel.calendar_theme_2025', jsonData),
      ]);
  ...
  Future<void> _loadSavedDates() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final String? savedData = prefs.getString('spaniel.calendar_theme_2025');
      final String? savedEventsData =
          prefs.getString('spaniel.calendar_events');

      // For backward compatibility, also check animal theme data if spaniel data is not found
      final String? animalSavedData =
          prefs.getString('spaniel.calendar_theme_2025');
      ...
      // First try to load spaniel specific events
      if (savedEventsData != null && savedEventsData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found spaniel event data in SharedPreferences, parsing...');
          ...
          decodedEvents.forEach((dateStr, eventsList) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date in spaniel events: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            debugPrint(
                '✅ Successfully loaded $loadedDates dates with $loadedEvents events from spaniel data');

            // If we found and loaded spaniel data, we're done
            if (mounted) {
              setState(() {});
            }
            return;
          ...
          debugPrint('❌ Error loading spaniel events data: $e');
        }
      }
      // Try spaniel legacy format
      else if (savedData != null && savedData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found spaniel legacy data in SharedPreferences, parsing...');
          ...
          decodedData.forEach((dateStr, category) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date from spaniel data: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Successfully loaded $loadedDates dates with $loadedEvents events from spaniel legacy format');
          ...
          debugPrint('❌ Error parsing spaniel legacy data: $e');
        }
      }

      // If we reach here, we didn't find any spaniel data. Now we can look for animal data for backward compatibility
      // and copy it to spaniel data for future use
      loadedDates = 0;
      loadedEvents = 0;
      ...
      if (animalSavedEventsData != null && animalSavedEventsData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found animal event data for backward compatibility, copying to spaniel...');
          final Map<String, dynamic> decodedEvents =
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Copied $loadedDates dates with $loadedEvents events from animal data to spaniel');

            // Save the data to spaniel keys for future use
            await _saveEvents();
            await _saveDates();
          }
        } catch (e) {
      ...
      // Check animal legacy format as last resort
        ...
        try {
          debugPrint(
              '📋 Found animal legacy data for backward compatibility, copying to spaniel...');
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Copied $loadedDates dates with $loadedEvents events from animal legacy format to spaniel');
        ...
        debugPrint(
            'ℹ️ No saved calendar data found in SharedPreferences for spaniel or animal themes');
      }
  ...
  static Future<void> backgroundCallback(Uri? uri) async {
    if (uri?.host == 'updatewidget') {
      ...
      final prefs = await SharedPreferences.getInstance();
      final String? data = prefs.getString('spaniel.calendar_data');

      if (data != null) {
        // Save spaniel data to widget
        await HomeWidget.saveWidgetData('spaniel.calendar_data', data);
        await HomeWidget.saveWidgetData('spaniel.calendar_theme_2025', data);
      }
  ...
  Future<void> _saveEvents() async {
    try {
      ...
      await prefs.setString('spaniel.calendar_events', eventsJson);
      debugPrint(
