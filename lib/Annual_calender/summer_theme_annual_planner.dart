...
class _SummerThemeCalendarAppState extends State<SummerThemeCalendarApp>
  ...
  Future<void> _saveDates() async {
    try {
      ...
      await Future.wait([
        prefs.setString('summer.calendar_theme_2025', jsonData),
        prefs.setString('summer.calendar_data', jsonData),
        HomeWidget.saveWidgetData('summer.calendar_data', jsonData),
        HomeWidget.saveWidgetData('summer.calendar_theme_2025', jsonData),
      ]);
  ...
  Future<void> _loadSavedDates() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final String? savedData = prefs.getString('summer.calendar_theme_2025');
      final String? savedEventsData = prefs.getString('summer.calendar_events');

      // For backward compatibility, also check animal theme data if summer data is not found
      final String? animalSavedData =
          prefs.getString('animal.calendar_theme_2025');
      final String? animalSavedEventsData =
          prefs.getString('animal.calendar_events');

      int loadedDates = 0;
      int loadedEvents = 0;

      // First try to load summer specific events
      if (savedEventsData != null && savedEventsData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found summer event data in SharedPreferences, parsing...');
          ...
          decodedEvents.forEach((dateStr, eventsList) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date in summer events: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            debugPrint(
                '✅ Successfully loaded $loadedDates dates with $loadedEvents events from summer data');

            // If we found and loaded summer data, we're done
            if (mounted) {
              setState(() {});
            }
            return;
          ...
          debugPrint('❌ Error loading summer events data: $e');
        }
      }
      // Try summer legacy format
      else if (savedData != null && savedData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found summer legacy data in SharedPreferences, parsing...');
          ...
          decodedData.forEach((dateStr, category) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date from summer data: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Successfully loaded $loadedDates dates with $loadedEvents events from summer legacy format');
          ...
          debugPrint('❌ Error parsing summer legacy data: $e');
        }
      }

      // If we reach here, we didn't find any summer data. Now we can look for animal data for backward compatibility
      // and copy it to summer data for future use
      loadedDates = 0;
      loadedEvents = 0;

      if (animalSavedEventsData != null && animalSavedEventsData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found animal event data for backward compatibility, copying to summer...');
          final Map<String, dynamic> decodedEvents =
              json.decode(animalSavedEventsData);
          ...
          decodedEvents.forEach((dateStr, eventsList) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date in animal events: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Copied $loadedDates dates with $loadedEvents events from animal data to summer');

            // Save the data to summer keys for future use
            await _saveEvents();
            await _saveDates();
          }
        } catch (e) {
          debugPrint('❌ Error copying animal events data: $e');
        }
      }
      // Check animal legacy format as last resort
      else if (animalSavedData != null && animalSavedData.isNotEmpty) {
        try {
          debugPrint(
              '📋 Found animal legacy data for backward compatibility, copying to summer...');
          final Map<String, dynamic> decodedData = json.decode(animalSavedData);
          ...
          decodedData.forEach((dateStr, category) {
            try {
              ...
              debugPrint(
                  '❌ Error parsing date from animal data: $dateStr - $e');
            }
          });
          ...
          if (parsedEvents.isNotEmpty) {
            ...
            events.addAll(parsedEvents);
            debugPrint(
                '✅ Copied $loadedDates dates with $loadedEvents events from animal legacy format to summer');
          ...
          debugPrint('❌ Error copying animal legacy data: $e');
        }
      } else {
        debugPrint(
            'ℹ️ No saved calendar data found in SharedPreferences for summer or animal themes');
      }
  ...
  static Future<void> backgroundCallback(Uri? uri) async {
    if (uri?.host == 'updatewidget') {
      ...
      final prefs = await SharedPreferences.getInstance();
      final String? data = prefs.getString('summer.calendar_data');

      if (data != null) {
        // Save summer data to widget
        await HomeWidget.saveWidgetData('summer.calendar_data', data);
        await HomeWidget.saveWidgetData('summer.calendar_theme_2025', data);
      }
  ...
  Future<void> _saveEvents() async {
    try {
      ...
      await prefs.setString('summer.calendar_events', eventsJson);
      debugPrint(
